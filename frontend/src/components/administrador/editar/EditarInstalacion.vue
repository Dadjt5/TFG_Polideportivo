        ...
        <div class="tab-content">
          ...
          <div class="tab-pane fade show active" id="info">
            <div class="row g-4">
              <div class="col-md-6">
                ...
                <input v-if="editando" type="text" class="form-control form-control-lg" v-model="instalacion.nombre" :class="{ 'is-invalid': errores.nombre }" />
              ...
              <div class="col-md-6">
                ...
                <select v-if="editando" class="form-select form-select-lg" v-model="instalacion.tipoInstalacion" :class="{ 'is-invalid': errores.tipoInstalacion }">
              ...
              <div class="col-md-4" v-if="instalacion.tipoInstalacion === 'PISCINA'">
                ...
                <input v-if="editando" type="number" min="1" class="form-control form-control-lg" v-model.number="instalacion.numeroCalles" :class="{ 'is-invalid': errores.numeroCalles }" />
                <div v-else class="form-control form-control-lg bg-light text-muted">{{ instalacion.numeroCalles }}</div>
              </div>

              <div class="col-md-4">
                <label class="form-label fw-semibold">{{ t.capacity }}</label>
                <input v-if="editando" type="number" min="1" class="form-control form-control-lg" v-model.number="instalacion.aforoMaximo" :class="{ 'is-invalid': errores.aforoMaximo }"/>
              ...
              <div class="col-md-4">
                ...
                <input v-if="editando" type="number" min="0" max="100" step="0.1" class="form-control form-control-lg" v-model.number="instalacion.porcentajeTDA" :class="{ 'is-invalid': errores.porcentajeTDA }"/>
                <div v-else class="form-control form-control-lg bg-light text-muted">{{ instalacion.porcentajeTDA }}%</div>
              </div>

              <div class="col-md-6">
                <label class="form-label fw-semibold">{{ t.pavilion || 'Pabellón' }}</label>
                <select v-if="editando" class="form-select form-select-lg" v-model="instalacion.pabellon.id" :class="{ 'is-invalid': errores.pabellon }">
                  <option value="-1" disabled>--</option>
                  <option v-for="p in pabellones" :key="p.id" :value="p.id">{{ p.nombre }}</option>
                </select>
                <div v-else class="form-control form-control-lg bg-light text-muted">{{ instalacion.pabellon?.nombre || '---' }}</div>
              </div>

              <div class="col-md-6">
                <label class="form-label fw-semibold">{{ t.tariff || 'Tarifa' }}</label>
                <select v-if="editando" class="form-select form-select-lg" v-model="instalacion.tarifa.id" :class="{ 'is-invalid': errores.tarifa }">
                  <option value="-1">--</option>
                  <option v-for="tar in tarifas" :key="tar.id" :value="tar.id">{{ tar.titulo }}</option>
                </select>
                <div v-else class="form-control form-control-lg bg-light text-muted">{{ instalacion.tarifa?.titulo || '---' }}</div>
              </div>
                ...
                <div class="card border-1 border-light shadow-sm rounded-4 h-100" :class="{'bg-light': !editando}">
                  <div class="card-body">
                    ...
                    <div v-if="dia.abierto">
                      ...
                      <div v-else class="d-flex align-items-center justify-content-center gap-2 py-2 bg-white rounded border">
          ...
          <div class="tab-pane fade" id="especiales">
            <div v-if="editando" class="mb-4">
              <button class="btn btn-primary rounded-pill px-4 shadow-sm" @click="fechasEspeciales.push({ fecha: '', horaApertura: '08:00', horaCierre: '22:00', abierto: true })">
              ...
              <div class="col-md-6 col-lg-4" v-for="(fecha, index) in fechasEspeciales" :key="index">
                <div class="card border-1 border-light shadow-sm rounded-4 p-3" :class="{'bg-light': !editando}">
                  <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <button v-if="editando" class="btn btn-sm btn-outline-danger" @click="fechasEspeciales.splice(index, 1)">✕</button>
                  </div>
                  <div class="mb-3">
                    ...
                    <span v-else class="badge" :class="fecha.abierto ? 'bg-success' : 'bg-danger'">{{ fecha.abierto ? 'Abierto' : 'Cerrado' }}</span>
                  </div>
          ...
          <div class="tab-pane fade" id="imagenes">
            <div class="d-flex justify-content-center">
              <div class="card shadow rounded-4 p-3 text-center" style="max-width: 500px; width: 100%; background-color: rgba(255,255,255,0.75); backdrop-filter: blur(8px);">
                <h5 class="mb-3 d-flex justify-content-center align-items-center gap-2">
                  <i class="bi bi-images text-primary"></i> {{ t.images }}
                </h5>

                <img v-if="instalacion.imagenURL && !preview" :src="instalacion.imagenURL" class="img-fluid rounded mb-3 shadow-sm" style="max-height: 300px; object-fit: cover;" />
                ...
                <img v-if="preview" :src="preview" class="img-fluid rounded shadow-sm border border-primary mt-3" style="max-height: 300px; object-fit: cover;" />
              </div>
            </div>
          </div>
